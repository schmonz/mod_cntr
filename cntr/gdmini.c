/*
 * A Trimmed version of gd that supports on-memory GIF rendering
 *
 * $Id: gdmini.c,v 2.1 1998/10/06 08:31:09 dankogai Exp $
 *
 */
#include <stdio.h>
#include <math.h>
...
dynamicPtr *gdImageGifData(gdImagePtr im)
{
...
static void BumpPixel(void)
{
    ...
    /*
     * If we are at the end of a scan line, set curx back to the beginning If
     * we are interlaced, bump the cury to the appropriate spot, otherwise,
     * just increment it.
     */
    if (curx == Width) {
	curx = 0;
...
static code_int maxmaxcode = (code_int) 1 << GIFBITS;	/* should NEVER generate
							 * this code */
...
{
    /*
     * Some of these are properly initialized later. What I'm doing here is
     * making sure code that depends on C's initialization of statics doesn't
     * break when the code gets called more than once.
     */
    Width = 0;
    Height = 0;
    curx = 0;
...
gdImageCreateFromGif(FILE *fd)
{
	...
	if (c == ';') {		/* GIF terminator */
	    ...
	    /*
	     * Check for open colors at the end, so we can reduce colorsTotal
	     * and ultimately BitsPerPixel
	     */
...
void gdImageFilledPolygon(gdImagePtr im, gdPointPtr p, int n, int c)
{
	...
	for (i = 0; (i < n); i++) {
		...
		(y - y1) * (x2 - x1) / (y2 - y1) + x1;
		/*
		 * Only count intersections once except at maxima and minima.
		 */
...
static int appendDynamic(dynamicPtr * dp, const void *src, int size)
{
    ...
    /*
     * if we get here, we can be sure that we have enough bytes to copy
     * safely
     */
    tmp = (char *) dp->data;
...
static int reallocDynamic(dynamicPtr * dp, int required)
{
    ...
    /*
     * First try realloc().  If that doesn't work, make a new memory block
     * and copy.
     */
...
void freeDynamic(dynamicPtr * dp)
{
    if (dp->data != NULL) {
	free(dp->data);
	dp->data = NULL;
